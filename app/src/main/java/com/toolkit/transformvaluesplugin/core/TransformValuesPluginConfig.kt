package com.toolkit.transformvaluesplugin.core
...
data class TransformValuesPluginConfig(
    ...
    fun getExcelFile() = File(excelPath)

    fun buildValuesStringsFile(index: Int) = mapLanguage[index]?.let { name ->
    ...
    fun isXlsFile() = excelPath.endsWith("xls", true)

    fun isXlsxFile() = excelPath.endsWith("xlsx", true)
